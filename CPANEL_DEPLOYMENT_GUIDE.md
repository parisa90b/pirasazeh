...
## نحوه تنظیم لینک گوگل شیت در کدها (اتصال دائم و بدون نیاز به پنل ادمین)
چون سایت به صورت استاتیک بیلد و روی هاست آپلود می‌شود، تنظیم لینک گوگل شیت از طریق مرورگر (LocalStorage) فقط روی همان کامپیوتر ذخیره می‌شود. 

برای اینکه لینک گوگل شیت **برای تمامی کاربران و بازدیدکنندگان وب‌سایت** فعال و دائمی باشد:
1. فایل **`src/siteConfig.ts`** را باز کنید.
2. در بخش شماره ۲ (حدود خط ۴۸)، متغیر **`DEFAULT_GOOGLE_SHEETS_CONFIG`** را پیدا کنید:
```typescript
export const DEFAULT_GOOGLE_SHEETS_CONFIG: GoogleSheetsConfig = {
  enabled: true,
  sheetIdOrUrl: 'https://docs.google.com/spreadsheets/d/شناسه_شیت_شما/edit', // 👈 لینک را اینجا بگذارید
  gallerySheetName: 'Gallery',   // یا 'گالری'
  articlesSheetName: 'Articles', // یا 'مقالات'
  autoSync: true,
};
```
3. لینک کامل گوگل شیت خود را در مقابل `sheetIdOrUrl` داخل کوتیشن قرار دهید و فایل را ذخیره کنید.
4. با دستور `npm run build` خروجی بگیرید و روی هاست آپلود کنید.
5. **از این پس، هر سطری به گوگل شیت اضافه کنید، بدون دست زدن به هاست یا کدها، بلافاصله در سایت برای همه کاربران نمایش داده می‌شود!**
